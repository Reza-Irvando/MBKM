...
from app.configs import serviceTransaction
from app import validators
from app.validators import transaction as TransVall
...
def budgetTopUp():
    try:
        ...
        path = f"{serviceTransaction}/transaction/unit/budget/topUp?userId={accessData['data']['userId']}"
...
def UnitBudgetBalance():
    try:
        ...
        ### start hit service ###
        path = f"{serviceTransaction}/transaction/unit/budget/balance?userId={accessData['data']['userId']}"
...
def UnitBudgetAllocation():
    try:
        ...
        ### start hit service ###
        bodyJson = request.json
        err = TransVall.BudgetUnitAllocation(bodyJson)
        if err:
            return responses.Make(
                Status=HTTPStatus.BAD_REQUEST.value,
                Message="error",
        ...
        path = f"{serviceTransaction}/transaction/unit/budget/allocation?userId={accessData['data']['userId']}&accountId={accessData['data']['accountId']}"
        responseAPI = httpx.post(path, json=bodyJson)
        responseJson = responseAPI.json()
        return responses.Make(Status=responseJson["status"], Message=responseJson["message"], Data=responseJson["data"]), 200
        ### stop hit service ###
