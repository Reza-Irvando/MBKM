...
def BankList():
    try:
        ...
        ### start hit service ###
        path = f"{serviceMaster}/master/bank/list"
...
def BankCreate():
    try:
        ...
        path = f"{serviceMaster}/master/bank/create?userId={accessData['data']['userId']}"
...
def BankDetail():
    try:
        ...
        path = f"{serviceMaster}/master/bank/detail?bankId={bankId}"
        responseAPI = httpx.get(path)
        responseJson = responseAPI.json()
        return responses.Make(Status=responseJson["status"], Message=responseJson["message"], Data=responseJson["data"]), 200
        ### stop hit service ###
...
def BankUpdate():
    try:
        ...
        bodyJson = request.json
        bankId = request.args["bankId"]
        path = f"{serviceMaster}/master/bank/update?userId={accessData['data']['userId']}&bankId={bankId}"
...
def BankDelete():
    try:
        ...
        ### start hit service ###
        bankId = request.args["bankId"]
        path = f"{serviceMaster}/master/bank/delete?userId={accessData['data']['userId']}&bankId={bankId}"
